...
def validate(dataloader, model, args, device):
    ...
    for i, (imgs, encoded_texts,  mask_batch_text,
        ...
        labels = labels.cuda()
        with torch.no_grad():
            mix_output, image_only_output, text_only_output, aux_output, irr_mean = model(imgs=imgs, encoded_texts=encoded_texts,
            ...
            loss_CE = F.cross_entropy(mix_output, labels)
            loss_CE_image = F.cross_entropy(image_only_output, labels)
            loss_CE_text = F.cross_entropy(text_only_output, labels)
            loss_single_modal = ( loss_CE_text + loss_CE_image) / 2
            test_loss = loss_CE + + 1.0 * loss_single_modal

            final_test_loss = final_test_loss + test_loss.item()
            real_labels = real_labels + labels.cpu().detach().clone().numpy().tolist()
            # the [1] is score
            predicted_labels = predicted_labels + (mix_output[:, 0] < mix_output[:, 1]).cpu().long().numpy().tolist()
...
def train_model(dataloader, model, optimizer, epoch, args):
    ...
    for i, (imgs, encoded_texts, mask_batch_text,
        ...
        labels = labels.cuda()
        # Gc_number_layer, 2, N;
        mix_output, image_only_output, text_only_output, aux_output, irr_mean = model(imgs=imgs, encoded_texts=encoded_texts,  mask_batch_T=mask_batch_text, mask_batch_TT=mask_T_T,
                mask_batch_TV=mask_T_V, adj_matrix=adj_matrix,
                               word_len=word_len,  word_spans=word_spans)
        # N, 2
        ...
        loss_CE_text = F.cross_entropy(text_only_output, labels)
        loss_single_modal = (loss_CE_text + loss_CE_image) / 2
        train_loss = loss_CE + + 1.0 * loss_single_modal
